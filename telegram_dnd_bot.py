...
def build_llm_prompt(messages: list[dict[str, str]], persona: str) -> list[dict[str, str]]:
    ...
    context_block = "\n".join(context_rows)
    latest_message = messages[-1] if messages else {"author": "Странница", "text": ""}
    return [
        {
            "role": "system",
            "content": (
                ...
                "и отголоски будущего. Тебе покровительствует Программа — туман, спокойствие и дорога. "
                f"Текущий оттенок голоса: {persona}. "
                "ВАЖНО: в первую очередь отвечай по смыслу сообщений, особенно последней реплики, "
                "а не абстрактной красивой метафорой. "
                "Допустима лёгкая мистичность, но ответ должен быть конкретным и уместным контексту. "
                "Если сообщение бессмысленное/неполное — прямо скажи, что смысл расплывчат, и задай 1 уточняющий вопрос. "
        ...
        },
        {
            "role": "user",
            "content": (
                "Вот свежие сообщения из чата:\n"
                f"{context_block}\n\n"
                "Ответь как Фэйт на ПОСЛЕДНЕЕ сообщение. "
                f"Последняя реплика от {latest_message['author']}: {latest_message['text']}"
            ),
        },
    ]
